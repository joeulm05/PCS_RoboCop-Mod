...
function Invoke-RcpInstall([string]$Package, $Game, $Manifest) {
    ...
    Assert-RcpCompatibility $Package $Game
    ...
    try {
        ...
        $priorTargets = Join-RcpPath $layout.Mods 'RoboCopGameplay/targets.txt'
        if (-not [IO.File]::Exists($priorTargets) -and $layout.MigrateLegacy) { $priorTargets = Join-RcpPath $layout.LegacyMods 'RoboCopGameplay/targets.txt' }
        if ([IO.File]::Exists($priorTargets) -and (Get-Item -LiteralPath $priorTargets).Length -le 4096) {
            $ids = [IO.File]::ReadAllText($priorTargets)
            $rows = @($ids -split '\r?\n' | Where-Object { $_.Length -gt 0 })
            $validIds = $rows.Count -le 64
            foreach ($row in $rows) {
                if ($row -notmatch '^\d{1,10} \d{1,10} \d{1,10} \d{1,10}$') { $validIds=$false; break }
                foreach ($word in ($row -split ' ')) { if ([uint64]$word -gt [uint32]::MaxValue) { $validIds=$false } }
            }
            if ($validIds) { [IO.File]::WriteAllText((Join-RcpPath $stageMod 'targets.txt'), ($rows -join "`n") + "`n", [Text.UTF8Encoding]::new($false)) }
        }
