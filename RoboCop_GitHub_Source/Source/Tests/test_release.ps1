...
if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT) {
    function Assert-RcpCompatibility([string]$Package, $Game) {
        & python3 (Join-Path $Package 'Source/Tools/check_game.py') $Game.Exe | Out-Host
        if ($LASTEXITCODE -ne 0) { throw 'Offline gameplay compatibility check failed.' }
    }
}
...
function Get-RcpExperimentalRelease { throw 'Deterministic test: bundled runtime only' }
