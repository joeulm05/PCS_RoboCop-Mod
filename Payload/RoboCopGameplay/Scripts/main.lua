-- RoboCop Gameplay v0.4.3 | Author: Joe "Gambit" Bradford
...
local function native(ids)
    if lastIDs==ids and ready then return true end
    writeIDs(ids)
...
local function service()
    ...
    if not ok then once('service-error','Update failed: '..tostring(err))end
end
local ok,err=pcall(function()
    RegisterLoadMapPreHook(function()loading=true;targets={};lastIDs=nil;missions={} end)
    RegisterLoadMapPostHook(function()loading=false end)
    assert(type(LoopInGameThreadWithDelay(1000,service))=='number','Game-thread timer unavailable')
end)
if not ok then log('INACTIVE: '..tostring(err))else log('v0.4.3 TEST loaded; 30-second patrol timer; portrait handled separately.')end
